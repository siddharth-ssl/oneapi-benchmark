...
run(size_t nx, size_t ny, size_t nz, size_t nbx, size_t nby, size_t nbz, size_t nt, MPI_Comm mpi_comm, sycl::device& d)
{
  ...
  std::cout << "Size = " << alloc_bytes*1e-9 << "GB" << std::endl;
  ...
  auto initialise_blocks = [&](sycl::handler& h) {
    h.parallel_for(sycl::nd_range<3>(threads, wg), [=](sycl::nd_item<3> it) {
      ...
      VARTYPE feq[NM * NG] = {0};
      VARTYPE rho = 1., ux = 0.1, uy = 0.1, uz = 0.1;
  ...
  // copy
  auto copy_blocks_d3q27 = [&](sycl::handler& h) {
    h.parallel_for(sycl::nd_range<3>(threads, wg), [=](sycl::nd_item<3> it) {
      const size_t bx = it.get_group(2);
      const size_t by = it.get_group(1);
      const size_t bz = it.get_group(0);
      ...
      const int z = it.get_local_id(0);
       	    ...
       	    b[idx(0,x,y,z,3,npx,npy,npz)] = bnew[idx(0,x,y,z,3,npx,npy,npz)];
            b[idx(1,x,y,z,3,npx,npy,npz)] = bnew[idx(1,x,y,z,3,npx,npy,npz)];
            b[idx(2,x,y,z,3,npx,npy,npz)] = bnew[idx(2,x,y,z,3,npx,npy,npz)];
            b[idx(3,x,y,z,3,npx,npy,npz)] = bnew[idx(3,x,y,z,3,npx,npy,npz)];
            b[idx(4,x,y,z,3,npx,npy,npz)] = bnew[idx(4,x,y,z,3,npx,npy,npz)];
            b[idx(5,x,y,z,3,npx,npy,npz)] = bnew[idx(5,x,y,z,3,npx,npy,npz)];
            b[idx(6,x,y,z,3,npx,npy,npz)] = bnew[idx(6,x,y,z,3,npx,npy,npz)];
